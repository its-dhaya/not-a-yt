    ...
    >
      {icon && (
        <Icon
          d={icon}
          size={13}
          className={active ? "text-emerald-400" : "text-zinc-400"}
        />
      )}
      <span className="flex-1 text-left">{label}</span>
      {active && (
        <Icon d="M9 12l2 2 4-4" size={12} className="text-emerald-400" />
      )}
      {kbd && !active && (
        <span className="text-[10px] text-zinc-500 mono">{kbd}</span>
      )}
    </button>
  );
}
    ...
    >
      ...
      {hasSel ? (
        <>
          {/* Duration badge */}
          <div
            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-zinc-800/80 border border-zinc-700/60
          ...
          {/* Crop */}
          <ToolBtn
            icon="M6.13 1L6 16a2 2 0 002 2h15M1 6.13l15-.13"
            label="Crop"
          />
          ...
          >
            {(close) => (
              <>
                <p className="text-[9px] text-zinc-500 uppercase tracking-widest px-3 py-1.5 font-semibold">
                  Video Filter
                </p>
                {FILTER_OPTIONS.map(({ label, css }) => (
                  <MenuItem
                    key={label}
                    label={label}
                    active={selectedClip.filter === css}
                    onClick={() => {
                      updateClip(selectedClip.id, { filter: css });
                      close();
                    }}
                  />
                ))}
          ...
          <Dropdown
            ...
            trigger={(open, toggle) => (
              <ToolBtn
                ...
                label={
                  selectedClip.speed !== 1 ? `${selectedClip.speed}×` : "Speed"
                }
                active={open}
                onClick={toggle}
              />
              ...
              <>
                ...
                {SPEED_OPTIONS.map(({ label, v }) => (
                  <MenuItem
                    key={v}
                    label={label}
                    active={selectedClip.speed === v}
                    onClick={() => {
                      updateClip(selectedClip.id, { speed: v });
                      close();
                    }}
                  />
                ))}
        ...
        <>
          ...
          <span className="text-[11px] text-zinc-600 italic px-2 shrink-0">
      ...
      {/* ── Save / Export ── */}
      <button
        className="flex items-center gap-1.5 text-[12px] font-medium text-zinc-400 border border-zinc-800
      ...
      {exportDone &&
        createPortal(
          <div
            className="fixed bottom-6 right-6 bg-zinc-900/98 backdrop-blur-xl border border-emerald-400/20
